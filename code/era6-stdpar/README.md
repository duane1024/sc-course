...
The same kernel using language-standard parallelism. Same source, multiple targets. Compile with NVHPC and you get GPU offload; compile with stock g++/clang and you get CPU threading + SIMD.

## C++23 / C++26 standard parallelism

```bash
# CPU multi-thread + SIMD
...
This is *the same idea* as Cray's auto-vectorization in 1976 — you describe the per-element computation, the compiler/runtime handles the parallelism — but at a *much* higher level of abstraction. It runs on:

- A 2025 ARM laptop CPU with NEON, threaded across cores.
- An x86 server with AVX-512, threaded across 96 cores.
- An NVIDIA GPU with SIMT warps.
- An AMD GPU when ROCm picks up `stdpar`.

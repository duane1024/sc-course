...
- **Total**: 1.194 EFLOPS LINPACK sustained — the first machine to break the exaflop barrier on HPL, June 2022.
...
The MI250X GPU is itself two chiplets (MI250 was the first NVIDIA-style "MCM GPU"). On the spec sheet you have 4 GPUs per node, but it's actually 8 graphics compute dies. The **exposed** unit is "Graphics Compute Die" (GCD); CUDA-equivalent code is written per-GCD, not per-package.
...
Aurora was originally announced in 2015 to use Intel Xeon Phi accelerators and to ship in 2018. It was redesigned multiple times when Xeon Phi was canceled (2018) and then again as Ponte Vecchio's schedule slipped. Aurora's eventual debut at #2 (behind Frontier) closed a chapter for Intel — Ponte Vecchio is, on the spec sheet, competitive with the MI250X, but Intel announced the discontinuation of the entire Data Center GPU Max product line in 2024 and pivoted to "Falcon Shores" (now itself canceled in favor of "Jaguar Shores"). The Aurora GPU is, for practical purposes, an end-of-line product.
...
- **Total**: 1.742 EFLOPS LINPACK (Nov 2024). Currently #1 on the Top500.
- **Interconnect**: Slingshot-11.
- **Power**: ~30 MW. ~58 GFLOPS/W on HPL — tied with Frontier for top efficiency among exascale systems.
- **Cooling**: direct liquid cooling.
...
- **Power**: 28 MW. ~16 GFLOPS/W — much *less* efficient than the GPU-based machines on HPL, but **#1 on HPCG** (real-application memory-bound benchmark) for several lists.
- **Cooling**: direct liquid cooling.

Fugaku is the architectural counter-argument. Rather than CPU + GPU, it goes "CPU only — but with vector units so wide and memory bandwidth so high that the GPU isn't needed." The A64FX chip has SVE 512-bit vectors (effectively two 8-wide FP64 lanes per cycle), and HBM2 memory directly attached, so per-core memory bandwidth is *higher than most GPUs*.

For the workloads that Japanese national institutions actually run — climate models, structural simulations, drug-discovery codes — Fugaku is faster *per real watt of useful work* than Frontier on a substantial subset. On HPL it's worse; on HPCG it's often better. The Earth Simulator's lesson, applied at exascale.
...
- **Fugaku**: ARM SVE 512-bit vectors, 2 lanes per chip.
...
3. **A modern code targeting GPUs**: written with one of:
   ...
   - **OpenMP target offload** (`#pragma omp target teams distribute parallel for`). Portable, ~80% of CUDA performance for typical kernels.
   - **Kokkos / Raja / SYCL**: portable C++ abstractions. ~85–95% of native.
...
In `labs/14-job-scripts/` we provide three real Slurm submission scripts — one for Frontier, one for Aurora, one for Fugaku — drawn from public documentation. They're heavily commented. You read them, identify the architectural assumptions baked in (per-node GPU counts, memory binding, interconnect tuning flags), and adapt one to run on a small Slurm cluster (which we set up in lab 10).
